 ...
 * not apply here — pointing them at the tests produces only false positives, because unit-test
 * paths mirror src and so inherit segment names that mean something different for a test. These
 * rules are the Java spelling of the PHPArkitect test rules the reference implementation enforces.
 *
...
public class TestArchitectureTest {
    ...
    private static JavaClasses importTestTree() {
        String property = System.getProperty("sindri.testTreeClasses");

        if (property == null || property.isBlank()) {
            throw new IllegalStateException(
                    "The sindri.testTreeClasses system property is not set; the test tree cannot be"
